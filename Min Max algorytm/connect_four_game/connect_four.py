...
class ConnectFour(Game):
    ...
    def __init__(self, size: Tuple[int, int] = (7, 6), first_player: Player = None,
...
class ConnectFourState(State):
    ...
    def __str__(self) -> str:
        ...
        text = '\n'.join(
            reversed([''.join(f'[{" " if field is None else field.char}]' for field in row
    ...
    def _check_four(self, start_coords: Tuple[int, int], move_coords: Tuple[int, int]) -> Optional[Player]:
        fields = {
            self.fields[start_coords[0] + move_coords[0] * i][
                start_coords[1] + move_coords[1] * i] for i in range(4)
        }
